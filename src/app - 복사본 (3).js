...
app.post('/profile/:userid/friendship/add', async (req, res) => {
    ...
    try {
        ...
        if (existingRequest.length === 0 && existingReceived.length === 0) {
            ...
            // 알림 추가
            await connection.execute('INSERT INTO notifications (user_id, message) VALUES (?, ?)', [receiverId, `${requesterId}님이 당신에게 서로이웃 요청을 보냈습니다.`]);
...
app.post('/profile/:userid/friendship/remove', async (req, res) => {
    ...
    try {
        ...
        // 해제 알림 추가
        await connection.execute('INSERT INTO notifications (user_id, message) VALUES (?, ?)', [friendId, `${userId}님이 당신과의 서로이웃 관계를 해제했습니다.`]);
