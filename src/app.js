...
// 친구 관계 상태 확인 함수
async function checkFriendshipStatus(userId, profileId) {
    const connection = await mysql.createConnection(dbConfig);

    // 요청자가 프로필 소유자에게 보낸 요청 상태 확인
    const [requesterToReceiver] = await connection.execute(
        'SELECT status FROM friend WHERE requester_id = ? AND receiver_id = ?',
        [userId, profileId]
    );

    // 수신자가 요청자에게 보낸 요청 상태 확인
    const [receiverToRequester] = await connection.execute(
        'SELECT status FROM friend WHERE requester_id = ? AND receiver_id = ?',
        [profileId, userId]
    );

    // 요청자가 보낸 요청이 있을 경우
    if (requesterToReceiver.length > 0) {
        return requesterToReceiver[0].status; // 요청자가 보낸 상태 반환
    } 
    // 수신자가 요청자에게 보낸 요청이 있을 경우
    else if (receiverToRequester.length > 0) {
        return receiverToRequester[0].status; // 수신자가 보낸 상태 반환
    }

    return null; // 관계가 없을 경우
}
...
app.get('/profile/:userid', async (req, res) => {
    ...
    try {
        ...
        if (userData.length > 0) {
            ...
            // 로그인한 경우 친구 상태 확인
            if (userId) {
                friendshipStatus = await checkFriendshipStatus(userId, profile.id);
            }

            res.render('profile', { user: req.session.user, profile, friendshipStatus }); // 사용자 프로필 렌더링
        } else {
...
app.get('/profile/:userid/notice', async (req, res) => {
    ...
    try {
        ...
        // 알림 페이지 렌더링
        res.render('notice', { user: req.session.user, notifications: notifications }); 
    } catch (error) {
...
// 게시물에 좋아요 추가/취소 라우트
app.post('/profile/:userid/article/like', async (req, res) => {
    const { postId } = req.body; // body에서 postId 가져오기
    const { userid } = req.params;
    ...
    try {
        ...
        // 현재 좋아요 수 가져오기
        const [likesCount] = await connection.execute('SELECT COUNT(*) AS count FROM likes WHERE post_id = ?', [postId]);
        
        res.json({ success: true, likesCount: likesCount[0].count, liked: existingLike.length === 0 }); // 응답으로 현재 좋아요 수와 좋아요 상태 반환
    } catch (error) {
...
// 게시물 저장 라우트
app.post('/profile/:userid/article/save', async (req, res) => {
    const { postId } = req.body; // POST 요청으로 전달된 postId를 가져옴
    const { userid } = req.params;
    ...
    try {
        ...
        // 이미 저장된 게시물인지 확인
        const [existingSavedPost] = await connection.execute('SELECT * FROM saved_posts WHERE user_id = ? AND post_id = ?', [userId, postId]);

        if (existingSavedPost.length === 0) {
            // 게시물 저장
            await connection.execute('INSERT INTO saved_posts (user_id, post_id) VALUES (?, ?)', [userId, postId]);
        }
...
app.get('/profile/:userid/saved', async (req, res) => {
    ...
    // 요청한 사용자와 현재 로그인한 사용자 확인
    if (req.session.user.username !== userid) {
        return res.redirect(`/profile/${req.session.user.username}`); // 자신의 프로필로 리디렉션
    }
    ...
    try {
        ...
        // 저장된 게시물 가져오기
        const [savedPosts] = await connection.execute(`
        ...
        res.render('saved', { user: req.session.user, savedPosts }); // 저장된 페이지 렌더링
    } catch (error) {
...
// 서로이웃 추가 요청 라우트
app.post('/profile/:userid/friendship/add', async (req, res) => {
    const { userid } = req.params;
    ...
    const receiverId = await getUserIdByUsername(userid); // username으로 userId 가져오기

    try {
        const connection = await mysql.createConnection(dbConfig);

        // 이미 요청한 경우 처리
        const [existingRequest] = await connection.execute('SELECT * FROM friend WHERE requester_id = ? AND receiver_id = ?', [requesterId, receiverId]);
        const [existingReceived] = await connection.execute('SELECT * FROM friend WHERE requester_id = ? AND receiver_id = ?', [receiverId, requesterId]);

        if (existingRequest.length === 0 && existingReceived.length === 0) {
            // 서로이웃 추가 요청 시 두 개의 레코드 생성
            await connection.execute('INSERT INTO friend (requester_id, receiver_id, status) VALUES (?, ?, ?)', [requesterId, receiverId, 'pending']);
            await connection.execute('INSERT INTO friend (requester_id, receiver_id, status) VALUES (?, ?, ?)', [receiverId, requesterId, 'received']);

            // 사용자 이름 가져오기
            const [requesterData] = await connection.execute('SELECT username FROM users WHERE id = ?', [requesterId]);
            const requesterUsername = requesterData[0].username;

            // 알림 추가
            await connection.execute('INSERT INTO notifications (user_id, message) VALUES (?, ?)', [receiverId, `${requesterUsername}님이 당신에게 서로이웃 요청을 보냈습니다.`]);
        } else if (existingReceived.length > 0) {
            return res.status(400).send('이미 요청이 존재합니다.');
        }
...
// 서로이웃 승인 요청 라우트
app.post('/profile/:userid/friendship/approve', async (req, res) => {
    const { userid } = req.params;
    ...
    const requesterId = await getUserIdByUsername(userid); // username으로 userId 가져오기

    try {
        const connection = await mysql.createConnection(dbConfig);

        // 요청을 승인
        await connection.execute('UPDATE friend SET status = ? WHERE requester_id = ? AND receiver_id = ?', ['accepted', requesterId, receiverId]);
        await connection.execute('UPDATE friend SET status = ? WHERE requester_id = ? AND receiver_id = ?', ['accepted', receiverId, requesterId]);
        ...
        // 승인 알림 추가
        await connection.execute('INSERT INTO notifications (user_id, message) VALUES (?, ?)', [requesterId, `${receiverId}님이 당신의 서로이웃 요청을 승인했습니다.`]);
...
// 서로이웃 해제 요청 라우트
app.post('/profile/:userid/friendship/remove', async (req, res) => {
    const { userid } = req.params;
    ...
    const userId = req.session.user.id;
    const friendId = await getUserIdByUsername(userid); // username으로 userId 가져오기

    try {
        const connection = await mysql.createConnection(dbConfig);

        // 서로이웃 관계 해제 시 상태를 rejected로 변경
        await connection.execute('UPDATE friend SET status = ? WHERE (requester_id = ? AND receiver_id = ?) OR (requester_id = ? AND receiver_id = ?)', ['rejected', userId, friendId, friendId, userId]);

        // 해제 알림 추가
        const [userData] = await connection.execute('SELECT username FROM users WHERE id = ?', [userId]);
        const username = userData[0].username;

        await connection.execute('INSERT INTO notifications (user_id, message) VALUES (?, ?)', [friendId, `${username}님이 당신과의 서로이웃 관계를 해제했습니다.`]);
