...
app.get('/profile/:userid/article', async (req, res) => {
    ...
    try {
        ...
        if (userData.length > 0) {
            ...
            res.redirect('/noprofile'); // 없는 경우 /noprofile로 리디렉션
        }
    } catch (error) {
...
// 게시물 삭제 라우트
app.post('/post/:postId/delete', async (req, res) => {
    const { postId } = req.params;

    if (!req.session.user) {
        return res.status(403).send('권한이 없습니다.'); // 로그인하지 않은 경우 접근 거부
    }

    try {
        const connection = await mysql.createConnection(dbConfig);
        await connection.execute('DELETE FROM posts WHERE id = ? AND user_id = ?', [postId, req.session.user.id]);
